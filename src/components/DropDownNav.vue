  ...
  <div class="fixed top-0 left-0 z-10 h-full w-full bg-bi-200 transition-all">
    <div class="mt-20 flex w-full flex-col items-start gap-5 text-black">
      <ul class="grid w-full grid-cols-2">
        <li class="col-span-2 mb-3 px-3 text-xl font-semibold text-bi-300">
          {{ $t(categories.name) }}
        </li>
        <li>
          <RouterLink
            class="block py-2 px-3 text-lg font-semibold hover:bg-bi-100"
            :to="`/${state.lang}/bids`"
          >
            {{ $t({ ar: 'الكل', en: 'all' }) }}
          </RouterLink>
      ...
      <ul class="w-full">
        <li class="mb-3 px-3 text-xl font-semibold text-bi-300">
          {{ $t(list.name) }}
        </li>
        <li
      ...
      <ul class="w-full">
        ...
        <li v-for="(item, index) in text.lang.languages" :key="index">
          <button
            class="flex w-full items-center gap-3 py-2 px-3 text-lg font-semibold hover:bg-bi-100"
